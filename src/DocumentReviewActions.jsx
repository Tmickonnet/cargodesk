  ...
  const handleStartReview = async () => {
    ...
    try {
      ...
      if (!data?.allowed || data?.new_status_code !== "UNDER_REVIEW") {
        setError(
          "The document review transition was not confirmed by the controlled workflow."
        );
        return;
      }
      ...
      if (onTransitioned) {
        onTransitioned(transitionedId, data);
      }
    } catch (transitionError) {
      setError(
        transitionError?.message || "Unable to start document review."
      );
    } finally {
    ...
    <>
      ...
      >
        ...
        <div style={{ fontSize: "12px", color: "#627d98", lineHeight: 1.6 }}>
          Start Review is available only for SUBMITTED documents and only when
          the existing DOCUMENT_VERIFY permission is granted. The database
          transition function remains authoritative.
        </div>
        ...
        >
          {rows
            .filter(
              (item) =>
                Number(item.document_status_id) === SUBMITTED_STATUS_ID
            )
            .map((item) =>
              canReview ? (
                <button
                ...
                >
                  Start Review{" "}
                  {item.document_number || "Document " + item.document_id}
                </button>
              ) : null
            )}
          ...
          {!canReview ? (
            <span style={{ fontSize: "12px", color: "#627d98" }}>
              Review action is not available for the current authorization
              context.
            </span>
          ) : null}
        </div>
      ...
      {selected ? (
        ...
        >
          <div
            style={{ fontSize: "13px", fontWeight: "700", color: "#173b6c" }}
          >
            Start review for{" "}
            {selected.document_number || "Document " + selected.document_id}?
          </div>
          ...
          >
            This invokes the existing controlled SUBMITTED → UNDER_REVIEW
            transition. No direct document update or audit insert is performed
            by the application.
          </div>
