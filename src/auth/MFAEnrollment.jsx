...
export default function MFAEnrollment({ onComplete }) {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      console.error(
        "CargoDesk MFA enrollment failed:",
        err
      );
  ...
  const handleVerify = async (event) => {
    ...
    if (!enrollment?.id) {
      setError(
        "MFA enrollment is not available. Please restart setup."
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "CargoDesk MFA verification failed:",
        err
      );
  ...
  return (
    <section>
      ...
      {enrollment.totp?.secret && (
        <div>
          <p>
            If you cannot scan the QR code, enter this
            setup key manually in your authenticator app:
          </p>
