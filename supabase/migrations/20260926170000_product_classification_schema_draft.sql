...
-- Scope:
--   - seven new tables
--   - structural constraints and narrowly justified indexes
--   - no existing table/data mutation
--   - no new SECURITY DEFINER functions
--   - no role-permission mapping changes
--
-- This file is intentionally a draft for review before any merge/application.
...
CREATE TABLE logistics.product (
    ...
    CONSTRAINT product_parent_fk FOREIGN KEY (parent_product_id)
        REFERENCES logistics.product(product_id) ON DELETE RESTRICT,
    CONSTRAINT product_not_self_parent_ck CHECK (
        parent_product_id IS NULL OR parent_product_id <> product_id
    ),
    CONSTRAINT product_effective_dates_ck CHECK (
...
CREATE TABLE logistics.classification_jurisdiction (
    ...
    CONSTRAINT classification_jurisdiction_country_fk FOREIGN KEY (country_id)
        REFERENCES logistics.countries(country_id) ON DELETE RESTRICT
);
...
CREATE TABLE logistics.classification_edition (
    ...
    CONSTRAINT classification_edition_system_fk FOREIGN KEY (classification_system_id)
        REFERENCES logistics.classification_system(classification_system_id) ON DELETE RESTRICT,
    CONSTRAINT classification_edition_status_ck CHECK (
        status_code IN ('ACTIVE','SUPERSEDED','RETIRED')
    ),
    CONSTRAINT classification_edition_dates_ck CHECK (
...
CREATE TABLE logistics.classification_record (
    ...
    CONSTRAINT classification_record_unique_key UNIQUE (
        classification_edition_id, classification_jurisdiction_id, classification_code
    ),
    CONSTRAINT classification_record_edition_fk FOREIGN KEY (classification_edition_id)
        REFERENCES logistics.classification_edition(classification_edition_id) ON DELETE RESTRICT,
    CONSTRAINT classification_record_jurisdiction_fk FOREIGN KEY (classification_jurisdiction_id)
        REFERENCES logistics.classification_jurisdiction(classification_jurisdiction_id) ON DELETE RESTRICT,
    CONSTRAINT classification_record_parent_fk FOREIGN KEY (parent_classification_record_id)
        REFERENCES logistics.classification_record(classification_record_id) ON DELETE RESTRICT,
    CONSTRAINT classification_record_status_ck CHECK (
        status_code IN ('ACTIVE','SUPERSEDED','RETIRED')
    ),
    CONSTRAINT classification_record_not_self_parent_ck CHECK (
        parent_classification_record_id IS NULL OR parent_classification_record_id <> classification_record_id
    )
);
...
CREATE TABLE logistics.product_classification (
    ...
    CONSTRAINT product_classification_product_fk FOREIGN KEY (product_id)
        REFERENCES logistics.product(product_id) ON DELETE RESTRICT,
    CONSTRAINT product_classification_record_fk FOREIGN KEY (classification_record_id)
        REFERENCES logistics.classification_record(classification_record_id) ON DELETE RESTRICT,
    CONSTRAINT product_classification_status_ck CHECK (
        status_code IN ('ACTIVE','SUPERSEDED','RETIRED')
    ),
    CONSTRAINT product_classification_dates_ck CHECK (
        effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from
    )
);
...
CREATE UNIQUE INDEX product_classification_active_unique_idx
    ON logistics.product_classification (product_id, classification_record_id)
    WHERE status_code = 'ACTIVE';

CREATE INDEX product_classification_record_lookup_idx
    ON logistics.product_classification (classification_record_id);
...
CREATE TABLE logistics.shipment_cargo_classification (
    ...
    CONSTRAINT shipment_cargo_classification_cargo_fk FOREIGN KEY (shipment_cargo_id)
        REFERENCES logistics.shipment_cargo(shipment_cargo_id) ON DELETE RESTRICT,
    CONSTRAINT shipment_cargo_classification_product_fk FOREIGN KEY (product_id)
        REFERENCES logistics.product(product_id) ON DELETE RESTRICT,
    CONSTRAINT shipment_cargo_classification_record_fk FOREIGN KEY (classification_record_id)
        REFERENCES logistics.classification_record(classification_record_id) ON DELETE RESTRICT,
    CONSTRAINT shipment_cargo_classification_verified_by_fk FOREIGN KEY (verified_by)
        REFERENCES logistics.users(user_id) ON DELETE RESTRICT,
    CONSTRAINT shipment_cargo_classification_status_ck CHECK (
        status_code IN ('SUGGESTED','UNDER_REVIEW','VERIFIED','REJECTED','SUPERSEDED')
    ),
    CONSTRAINT shipment_cargo_classification_source_ck CHECK (
        source_code IN ('MASTER_REFERENCE','HUMAN_ENTERED','HUMAN_VERIFIED','IMPORTED_REFERENCE','DOCUMENT_DERIVED','SYSTEM_SUGGESTED')
    ),
    CONSTRAINT shipment_cargo_classification_verification_ck CHECK (
        (status_code = 'VERIFIED' AND verified_by IS NOT NULL AND verified_at IS NOT NULL)
        OR
...
CREATE INDEX shipment_cargo_classification_cargo_lookup_idx
    ON logistics.shipment_cargo_classification (shipment_cargo_id);

CREATE INDEX shipment_cargo_classification_product_lookup_idx
    ON logistics.shipment_cargo_classification (product_id);

CREATE INDEX shipment_cargo_classification_record_lookup_idx
    ON logistics.shipment_cargo_classification (classification_record_id);

COMMENT ON TABLE logistics.product IS 'Product and sub-product hierarchy for shipment cargo identity; initially read-only operational reference data.';
...
-- SECURITY / RLS / GRANTS / RBAC are intentionally excluded from this draft.
-- They require separate final review before executable implementation.
